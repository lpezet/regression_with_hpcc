...
The SAS *raw* file is [elemapi.sas7bdat](https://stats.idre.ucla.edu/wp-content/uploads/2016/02/elemapi-1.sas7bdat). It has been converted to CSV in order to load it into HPCC.
This is the file we will be using from now on until mentioned otherwise.
...
Before running any analysis, it's best to understand what kind of data we are dealing with and its quality.
...
Load and submit the `Chap1_1_ExaminingData.ecl` file and let's take a look at the quality of the data now.
...
I provided a module called `Profiler` that will help with this task and shows results in the next outputs.
 
## 1.1.2 All Variables
 
The `AllVariables` output shows information for all variables.
This is generated by the action below:

```
Profiler.profile_fields( oProfileAllVariables, oDS );
OUTPUT( oProfileAllVariables, NAMED('AllVariables') );
```
...
It is now very easy to see now if (where) we are missing values (empty).
Any variable with less than 400 observations is a variable with at least one missing value.

This type of output can help verify data definitions provided with the data. 
For instance, a date field in the provided data definition might say it's 8 characters long (something like YYYYMMdd).
But the output here might say that the maximum length of this field is different than `min_value_length`, like 10 for example, which shows inconsistencies.
...
```
Profiler.profile_select_fields_values( oProfileSelectVariablesValues, oDS, 'full,meals,acs_k3,api00' );
OUTPUT( oProfileSelectVariablesValues, NAMED('SelectFieldsValues') );
```
...
While the previous outputs helped understand where values were missing, this one provides a little more information on actual values by variable and their frequency.
For instance, we knew already that the variable `acs_k3` had values missing (since its ``obs = 398`` ) but here we also see some negative values as well.
